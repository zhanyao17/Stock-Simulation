        ...
        for (index,d) in records.iter_mut().enumerate(){
            if d.stock_name == stock_name{
                ...
                if current_stock_price <= d.cut_loss {
                    let loss_rate = ((d.cut_loss - current_stock_price) * 100.00).round();
                    println!("{}Broker 1: Had sold User {}'s [{}] for cutting loss! [with ↓{}%]{}",ANSI_BOLD_RED,d.id,d.stock_name,loss_rate,ANSI_RESET);
                    to_remove.push(index);
                    sold_stocks.push((d.stock_name.clone(),d.num_stock));
                }else if current_stock_price >= d.take_profit   {
                    let earn_rate = ((current_stock_price - d.take_profit) * 100.00).round();
...
fn main() -> Result<()> {
    ...
    loop {
        // Get order list
        for (_, stock_list) in exch_brk1_stock_list.receiver().iter().enumerate() {
            match stock_list {
                    ...
                    match message {
                        ConsumerMessage::Delivery(delivery) => {
                            ...
                            match chosen_stock {
                                ...
                                None => {
                                    println!("{}Broker 1: shares were overpriced for User {}'s order!{}", ANSI_BOLD_RED, user_list.id, ANSI_RESET);
                                }
                            }
                ...
                Ok(stocks) => {
                    match stocks {
                        ConsumerMessage::Delivery(delivery) => {
                            ...
                            let sold_result :Vec<(String,i128)> = PurchaseDetails::stock_sell_monitoring(stock_profile.0, stock_profile.1);
                            // #NOTE: stock sell info send back to exchanges
